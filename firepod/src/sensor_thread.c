...
{
    // Using ependency injection here instead of statically defined queues. As this project gets built out, there 
    struct k_msgq *q = (struct k_msgq *)a1;

    struct k_sem *lora_trigger_sem = (struct k_sem *)a2;

    // configure Bosch API -> function pointer assignment
    ...
    // AI suggested this kind of formatting, seems good
    ...
    // configuration to determine how many measurements that will be requested and if there will be a filter
    ...
    while (1)
    {
        ...
        // converting float to fixed point integers because printk doesn't support printing floats, we also don't want to use the FPU too much
        ...
        // again, lengthy operations to manually do what the computer normally would do to print floats
        ...
        // put the out data in the queue for the LoRa thread
